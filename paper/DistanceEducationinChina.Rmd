...
knitr::include_graphics("../figures/frame_distance.png")
```
...
names_coef <- c(
  ...
  "`presentation_teacher&slides`" = "多元呈现形式"
)
...
rm10 <- broom::tidy(m10) %>% 
  filter(!str_detect(term, "num_civics|gender|political_status|major|grade|GPA|YoB|university_like"))%>% 
  mutate(model = "认同增强：中央政府施政满意度")

rm11 <- broom::tidy(m11) %>% 
  filter(!str_detect(term, "num_civics|gender|political_status|major|grade|GPA|YoB|university_like"))%>% 
  mutate(model = "认同增强：中央政府信任")
...
```{R, fig.cap="教师呈现对远程思政课教学效果的影响", out.width = '100%', fig.pos="h"}
