...
思想政治教育课程是中国政治宣传的重要方式。中国共产党从革命、建设到改革各个历史时期都高度重视思想政治建设，从革命时期的陕北公学到新中国成立后在中学开设"中国革命常识"、"共同纲领"等课程；再到改革开放后，中共先后出台10多个关于学校思想政治工作的文件，对思政课建设提出明确要求，不断推动思政课改革。思想政治教育课程作为中国大学生的必修课程，是落实立德树人根本任务的关键课程。[@XiJinPing2020]。 在互联网的普及和疫情防控的新形势下，在线教育以其较高的教学灵活性、较低的教育成本、优质资源的共享性等诸多优势成为疫情治理新常态下思政教育的重要支撑。 [@YuanMingZeEtAl2020a] 在线教育对思政课的在内容、媒介和形式上都提出了新的要求，"思政课不能拿着文件宣读，没有生命、干巴巴的"[^1]。
...
kbl(table1, booktabs = T, align = "c", caption = "实验干预与被试感知一致性程度") %>%
...
#    scale_color_gb(reverse = TRUE) + 
    theme(text = element_text(family='SimSong', face = "bold", colour = "black"), axis.title.x = element_text(family='Times', hjust = 0.5), legend.text = element_text(family='SimSong', size = 10)) + 
...
msummary(models,
         ...
         coef_map = names_coef,
         stars = TRUE,
         title = "网络思政课呈现内容和呈现方式对思政课教学效果的影响"
         )
...
plot_DoEs <- {
  dwplot(rbind(rm9, rm10, rm11, rm12),
    ...
    xlab("Coefficient Estimate") + 
#    scale_color_gb(reverse = TRUE) + 
    theme(text = element_text(family='SimSong', face = "bold", colour = "black"), axis.title.x = element_text(family='Times'), ) + 
    geom_vline(aes(xintercept=0), colour = "grey60", linetype = 2)
}
...
plot_DoEs <- {
  dwplot(rbind(rm10, rm11),
    ...
    theme_bw() + 
    xlab("回归系数（国家能力综合感知）") + 
    theme(text = element_text(family='SimSong', face = "bold", colour = "black"), axis.title.x = element_text(family='SimSong'), ) + 
    geom_vline(aes(xintercept=0), colour = "grey60", linetype = 2) + 
    scale_colour_grey(
        start = .1,
        end = .1,
    ...
    guides(
        shape = guide_legend("模型"), 
        colour = guide_legend("模型"))
}

ggsave("../figures/Model1.png", plot_DoEs, width = 8, height = 4.5)
...
ggsave("../figures/Model3.png", plot_DoEs, width = 8, height = 4.5)
...
plot_DoEs <- {
  dwplot(rbind(rm9, rm10, rm11, rm12),
    ...
    scale_colour_grey(
        ...
        # if start and end same value, use same colour for all models
        name = "模型",
        labels = c("国家能力综合感知", "中央政府施政满意度", "中央政府信任", "课程偏好")
    ) +
    scale_shape_discrete(
        name = "模型",
        labels = c("国家能力综合感知", "中央政府施政满意度", "中央政府信任", "课程偏好")
...
ggsave("../figures/Model.png", plot_DoEs, width = 8, height = 4.5)
```
