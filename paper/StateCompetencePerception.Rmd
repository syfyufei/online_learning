...
```{R frame, fig.cap="本文的分析框架", out.width = '100%', fig.pos="h"}
...
table2 <- data.frame("干预类别" = c("第一组（课程思政）", "第二组（传统思政课程）"),
"样本覆盖率" = c("50.14%", "49.86%")) %>% 
  mutate_all(linebreak)

kbl(table2, booktabs = T, align = "c", caption = "思想政治课的两种干预状态") %>%
...
```{R, eval = FALSE}
content_knowledge <- data %>% 
  dplyr::filter(content_knowledge == 1)

content_major <- data %>% 
  dplyr::filter(content_major == 1)

content_propaganda <- data %>% 
  dplyr::filter(content_propaganda == 1)
...
m1 <- lm(SA ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), data = content_major)
m2 <- glm(SA_certification ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
m3 <- glm(SA_coercive ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
m4 <- glm(SA_enrich ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
m5 <- glm(SA_rules ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
m6 <- glm(SA_draw ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
m7 <- glm(SA_command ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
m8 <- glm(SA_redistribution ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_major)
...
ggsave("../figures/content_major.png", plot_content_major, width = 8, height = 4.5)

# content_propaganda

m1 <- lm(SA ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), data = content_propaganda)
m2 <- glm(SA_certification ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
m3 <- glm(SA_coercive ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
m4 <- glm(SA_enrich ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
m5 <- glm(SA_rules ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
m6 <- glm(SA_draw ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
m7 <- glm(SA_command ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
m8 <- glm(SA_redistribution ~ knowlege_score + num_civics + gender + political_status + major + grade + GPA + YoB + university_like + as.factor(video_type), family = quasipoisson(link = "log"), data = content_propaganda)
...
ggsave("../figures/content_propaganda.png", plot_content_propaganda, width = 8, height = 4.5)

all <- (plot_content_major|plot_content_propaganda) + plot_layout(guides = 'collect')

ggsave("../figures/all.png", all, width = 8, height = 4.5)

```

```{R, fig.cap="思想政治教育对学生各类国家能力感知的影响", out.width = '100%', fig.pos="h"}
